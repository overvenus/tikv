...
impl BatchEntryIterator {
    ...
    pub fn next(&mut self) {
        ...
        if self.cursor_offset >= self.files[self.cursor_file].1 && self.cursor_file + 1 < self.files.len() {
            self.cursor_file += 1;
    ...
    fn read_record(&mut self) -> Option<EntryBatch> {
        ...
        if self.cursor_file >= self.files.len() || self.cursor_offset + 4 > self.files[self.cursor_file].1 {
            return None
        }
...
//                 self.cursor_offset, self.files[self.cursor_file].1);
        if !pread_int(self.files[self.cursor_file].0, self.cursor_offset as u64, &mut record_len) {
            return None
        }
        ...
        if !pread(self.files[self.cursor_file].0, &mut buf, self.cursor_offset as u64 + 4, record_len as u64) {
            return None
        }
    ...
    unsafe {
        loop {
            ...
            if ret_size < 0 {
                ...
                panic!("pread in {}, len: {}, failed, err {}", offset, len, err.to_string());
            }
...
pub fn seek_to_end(fd: libc::c_int) {
    unsafe  {
...
mod tests {
    ...
    fn test_iterator() {
        ...
        for region_id in 1..3 {
            ...
            batch.write_to_vec(&mut content);
            assert_eq!(batch.compute_size() as usize, content.len());
            println!("write content({} bytes) in offset: {}", content.len() + 4, file_size);
