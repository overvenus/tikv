...
use crate::storage::{
    ...
    Result as StorageResult, Snapshot, metrics::OLD_VALUE_INSERTION,
};
    ...
    fn process_write(mut self, snapshot: S, context: WriteContext<'_, L>) -> Result<WriteResult> {
        ...
        let (pr, to_be_write, rows, ctx, lock_info) = if res.is_ok() {
            ...
            if !self.old_values.is_empty() {
                OLD_VALUE_INSERTION.with_label_values(&["acquire_pessimistic_lock"]).inc_by(self.old_values.len() as i64);
            }
            let extra = TxnExtra {
