...
impl Debugger {
    ...
    pub fn set_region_tombstone_by_id(&self, regions: Vec<u64>) -> Result<Vec<(u64, Error)>> {
        ...
        for region_id in regions {
            ...
            let region_state = match db.get_msg::<RegionLocalState>(&key) {
                Ok(Some(state)) => state,
                Ok(None) => {
...
mod tests {
    ...
    fn test_tombstone_regions_by_id() {
        ...
        // region 1 with peers at stores 11, 12, 13.
        init_region_state(&debugger.engines, 1, &[11, 12, 13]);
        let mut expected_state = get_region_state(&debugger.engines, 1);
        expected_state.set_state(PeerState::Tombstone);

        // tombstone region 1.
        let errors = debugger.set_region_tombstone_by_id(vec![1]).unwrap();
        assert!(errors.is_empty());
        assert_eq!(get_region_state(&debugger.engines, 1), expected_state);
